...
func (kv *ShardKV) ShardMigration(args *MigrateArgs, reply *MigrateReply) {
	...
	// 每个集群只有leader才可以，负责读取shard信息，并且进行发送
	if _, isLeader := kv.rf.GetState(); !isLeader {
		return
	}
	kv.mu.Lock()
...
// 定期检查配置是否更新，如果更新，拉去最新的配置
func (kv *ShardKV) tryPollNewCfg() {
	_, isLeader := kv.rf.GetState()
	kv.mu.Lock()
	// 配置更新中，必须等待上一个的配置更新完成，再进行下一个的配置更新
...
func (kv *ShardKV) tryPullShard() {
	// 拉取shard本质上都是leader来进行负责的，
	// follower只是负责apply masg以及leader sync的状态
	_, isLeader := kv.rf.GetState()
	kv.mu.Lock()
	// 是否是leader，不是leader不可以拉取配置信息
	// 是否存在需要拉取的配置，如果不存在返回
	if !isLeader || len(kv.comeInShards) == 0 {
		kv.mu.Unlock()
		return
	...
	for shard, idx := range kv.comeInShards {
		wait.Add(1)
		// 确保每个集群的服务器都是同步更新的
		// 实际上只有leader更新，follower和leader sync
		go func(shard int, cfg shardmaster.Config) {
			defer wait.Done()
			args := MigrateArgs{shard, cfg.Num}
			gid := cfg.Shards[shard]
